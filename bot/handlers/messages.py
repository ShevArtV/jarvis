...
async def _get_or_start_persistent_worker(chat, thread_id: int, key: tuple[int, int]):
    """Живой процесс топика: существующий или поднятый заново.

    Возвращает ``(worker, spawned)``; ``(None, False)`` — persistent для
    текущего движка топика не действует, вызывающий идёт обычным путём.
    Ошибка запуска пробрасывается. Всё под per-key локом: на топик — строго
    один процесс, сколько бы ходов (сообщения, job'ы, триггеры) ни пришло разом."""
    lock = persistent_spawn_locks.setdefault(key, asyncio.Lock())
    async with lock:
...
async def _handle_persistent_message(
    ...
    try:
        worker, spawned = await _get_or_start_persistent_worker(chat, thread_id, key)
    except Exception as exc:
        logger.exception("persistent worker spawn failed key=%s", key)
        await send_to_topic(
            chat, thread_id,
            f"⚠️ Не удалось поднять живой процесс {get_session(*key)[2]}: {exc}",
        )
        return
    if worker is None:
